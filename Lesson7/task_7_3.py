...
Подсказака: https://pythonworld.ru/osnovy/peregruzka-operatorov.html 
"""
...
class Cell:
    ...
    def __truediv__(self, other: 'Cell'):
        self.validate_item(other)
        return Cell(self.count // self.count)

    def __str__(self):
        return f"The cell consists of {self._count} cells"

    def validate_item(self, other):
        assert isinstance(
            other, self.__class__), "Operations are allowed only between cells"
...
print(Cell.make_order(all_cells, 5))
